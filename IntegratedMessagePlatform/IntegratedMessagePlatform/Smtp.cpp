...
bool Smtp::AddAttachment(const char *path)
{
	std::string str(path);
...
bool Smtp::AddRecipient(const char *email, const char *name)
{
	assert(email);
	if(!email)
	{
...
bool Smtp::AddCCRecipient(const char *email, const char *name)
{
	assert(email);
...
bool Smtp::AddBCCRecipient(const char *email, const char *name)
{
	assert(email);
...
SOCKET Smtp::ConnectRemoteServer(const char *server,const unsigned short port)
{
	short nProtocolPort;
	LPHOSTENT lpHostEnt;
	LPSERVENT lpServEnt;
...
bool Smtp::FormatHeader(char* header)
{
	int i,s = 0;
	TCHAR szDate[500];
	TCHAR sztTime[500];
...
CSmtpError Smtp::GetLastError()
{
	return m_oError;
}

const char* const Smtp::GetLocalHostName() 
{
	if(m_pcLocalHostName)
		delete[] m_pcLocalHostName;
...
unsigned const int Smtp::GetBCCRecipientCount()
{
	return BCCRecipients.size();
}

unsigned const int Smtp::GetCCRecipientCount() 
{
	return CCRecipients.size();
}

const char* const Smtp::GetMessageBody() 
{
	return m_pcMsgBody;
}

unsigned const int Smtp::GetRecipientCount()
{
	return Recipients.size();
}

const char* const Smtp::GetReplyTo()  
{
	return m_pcReplyTo;
}

const char* const Smtp::GetMailFrom() 
{
	return m_pcMailFrom;
}

const char* const Smtp::GetSenderName() 
{
	return m_pcNameFrom;
}

const char* const Smtp::GetSubject() 
{
	return m_pcSubject;
}

const char* const Smtp::GetXMailer() 
{
	return m_pcXMailer;
}

CSmptXPriority Smtp::GetXPriority()
{
	return m_iXPriority;
}

void Smtp::SetXPriority(CSmptXPriority priority)
{
	m_iXPriority = priority;
}

void Smtp::SetMessageBody(const char *body)
{
	assert(body);
	int s = strlen(body);
	if (m_pcMsgBody)
...
void Smtp::SetReplyTo(const char *replyto)
{
	assert(replyto);
...
void Smtp::SetSenderMail(const char *email)
{
	assert(email);
	int s = strlen(email);
	if (m_pcMailFrom)
...
void Smtp::SetSenderName(const char *name)
{
	assert(name);
	int s = strlen(name);
	if (m_pcNameFrom)
...
void Smtp::SetXMailer(const char *xmailer)
{
	assert(xmailer);
...
void Smtp::SetLogin(const char *login)
{
	assert(login);
	int s = strlen(login);
	if (m_pcLogin)
...
void Smtp::SetSMTPServer(const char* SrvName,const unsigned short SrvPort)
{
	assert(SrvName);
